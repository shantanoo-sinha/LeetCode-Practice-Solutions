...
class Solution {
    public int[] findOrder(int numCourses, int[][] prerequisites) {
        ...
        for(int i=0; i < numCourses; i++) {
            if(!visited[i] && traverse(i, adj, stack, visited))
                return new int[0];
        }
        
        int i = stack.size();
        int[] order = new int[i];
        for(Integer course : stack) {
            order[--i] = course;
        }
    ...
    private boolean traverse(int node, List<Integer>[] adj, Set<Integer> stack, boolean[] visited) {
        visited[node] = true;
        
        for(int neighbor : adj[node]) {
            if(visited[neighbor] && !stack.contains(neighbor))
                return true;
            if(traverse(neighbor, adj, stack, visited))
                return true;
        }
        stack.add(node);
        return false;
